...
class FSPSSedModeler(Modeler):
    ...
    config_options.update(
        ...
        vactoair_flag=Param(
            bool, False, msg="If True, output wavelengths in air (rather than vac)"
        ),
        zcontinuous=Param(
            int, 1, msg="Flag for interpolation in metallicity of SSP before CSP"
        ),
        add_agb_dust_model=Param(
            bool,
            True,
            msg="Turn on/off adding AGB circumstellar dust contribution to SED",
        ),
        add_dust_emission=Param(
            bool, True, msg="Turn on/off adding dust emission contribution to SED"
        ),
        add_igm_absorption=Param(
            bool, False, msg="Turn on/off adding IGM absorption contribution to SED"
        ),
        add_neb_emission=Param(
            bool, False, msg="Turn on/off nebular emission model based on Cloudy"
        ),
        add_neb_continuum=Param(
            bool, False, msg="Turn on/off nebular continuum component"
        ),
        add_stellar_remnants=Param(
            bool,
            True,
            msg="Turn on/off adding stellar remnants contribution " "to stellar mass",
        ),
        compute_light_ages=Param(
            bool,
            False,
            msg=" If True then the returned spectra are actually "
            "light-weighted ages (in Gyr)",
        ),
        nebemlineinspec=Param(
            bool, False, msg="True to include emission line fluxes in spectrum"
        ),
        smooth_velocity=Param(
            bool, True, msg="True/False for smoothing in " "velocity/wavelength space"
        ),
        smooth_lsf=Param(
            bool,
            False,
            msg="True/False for smoothing SSPs by a wavelength dependent "
            "line spread function",
        ),
        cloudy_dust=Param(
            bool, False, msg="Switch to include dust in the Cloudy tables"
        ),
        agb_dust=Param(float, 1.0, msg="Scales the circumstellar AGB dust emission"),
        tpagb_norm_type=Param(
            int,
            2,
            msg="Flag for TP-AGB normalization scheme, "
        ...
        redgb=Param(
            float,
            1.0,
            msg="Modify weight given to RGB. Only available with BaSTI "
            "isochrone set",
        ),
        ...
        fcstar=Param(
            float,
            1.0,
            msg="Fraction of stars that the Padova isochrones identify "
            "as Carbon stars",
        ),
        ...
        stellar_metallicities_key=Param(
            str,
            "stellar_metallicity",
        ...
        pmetals_key=Param(
            str,
            "pmetals",
            msg="The power for the metallicty distribution function,"
            "only used if zcontinous=2, dataset keyword name",
        ),
        imf_type=Param(int, 1, msg="IMF type, see FSPS manual, default Chabrier IMF"),
        imf_upper_limit=Param(
            float, 120.0, msg="The upper limit of the IMF in solar masses"
        ),
        imf_lower_limit=Param(
            float, 0.08, msg="The lower limit of the IMF in solar masses"
        ),
        imf1=Param(
            float, 1.3, msg="log slope of IMF in 0.08<M/Msun<0.5, if imf_type=2"
        ),
        ...
        vdmc=Param(
            float,
            0.08,
            msg="IMF parameter defined in van Dokkum (2008). "
            "Only used if imf_type=3",
        ),
        mdave=Param(
            float,
            0.5,
            msg="IMF parameter defined in Dave (2008). Only used if imf_type=4.",
        ),
        evtype=Param(
            int,
            -1,
            msg="Compute SSPs for only the given evolutionary type. "
        ...
        gas_metallicities_key=Param(
            str,
            "gas_metallicity",
            msg="gas metallicities (log10(Zgas / Zsun)) dataset " "keyword name",
        ),
        igm_factor=Param(float, 1.0, msg="Factor used to scale the IGM optical depth"),
        sfh_type=Param(
            int, 0, msg="star-formation history type, see FSPS manual, default SSP"
        ),
        tau_key=Param(
            str,
            "tau",
            msg="Defines e-folding time for the SFH, in Gyr. Only used if "
            "sfh=1 or sfh=4, dataset keyword name",
        ),
        const_key=Param(
            str,
            "const",
            msg="Defines the constant component of the SFH, Only used if"
            " sfh=1 or sfh=4, dataset keyword name",
        ),
        sf_start_key=Param(
            str,
            "sf_start",
            msg="Start time of the SFH, in Gyr. Only used if sfh=1 or "
            "sfh=4 or sfh=5, dataset keyword name",
        ),
        sf_trunc_key=Param(
            str,
            "sf_trunc",
            msg="Truncation time of the SFH, in Gyr. Only used if "
            "sfh=1 or sfh=4 or sfh=5, dataset keyword name",
        ),
        stellar_ages_key=Param(
            str, "stellar_age", msg="galaxy stellar ages (Gyr)," "dataset keyword name"
        ),
        fburst_key=Param(
            str,
            "fburst",
            msg="Deﬁnes the fraction of mass formed in an instantaneous "
        ...
        tburst_key=Param(
            str,
            "tburst",
            msg="Defines the age of the Universe when the burst occurs."
        ...
        sf_slope_key=Param(
            str,
            "sf_slope",
            msg="For sfh=5, this is the slope of the SFR after time "
            "sf_trunc, dataset keyword name",
        ),
        dust_type=Param(
            int,
            2,
            msg="attenuation curve for dust type, see FSPS manual, " "default Calzetti",
        ),
        dust_tesc=Param(
            float,
            7.0,
            msg="Stars younger than dust_tesc are attenuated by both dust1 and"
            " dust2, while stars older are attenuated by dust2 only. "
            "Units are log(yrs)",
        ),
        dust_birth_cloud_key=Param(
            str,
            "dust1_birth_cloud",
            msg="dust parameter describing young stellar light attenuation "
            "(dust1 in FSPS), dataset keyword name",
        ),
        dust_diffuse_key=Param(
            str,
            "dust2_diffuse",
            msg="dust parameters describing old stellar "
        ...
        dust_clumps=Param(
            int,
            -99,
            msg="Dust parameter describing the dispersion of a Gaussian PDF"
        ...
        frac_nodust=Param(
            float,
            0.0,
            msg="Fraction of starlight that is not attenuated by the diffuse"
            " dust component",
        ),
        frac_obrun=Param(
            float,
            0.0,
            msg="Fraction of the young stars (age < dust_tesc) that are not "
        ...
        dust_index_key=Param(
            str,
            "dust_index",
            msg="Power law index of the attenuation curve. "
            "Only used when dust_type=0, dataset keyword "
            "name",
        ),
        dust_powerlaw_modifier_key=Param(
            str,
            "dust_calzetti_modifier",
        ...
        mwr_key=Param(
            str,
            "mwr",
            msg="The ratio of total to selective absorption which characterizes "
        ...
        uvb_key=Param(
            str,
            "uvb",
            msg="Parameter characterizing the strength of the 2175A extinction "
        ...
        wgp1_key=Param(
            str,
            "wgp1",
            msg="Integer specifying the optical depth in the Witt & Gordon "
        ...
        wgp2=Param(
            int,
            1,
            msg=" Integer specifying the type of large-scale geometry and "
            "extinction curve. Values range from 1-6, used only when dust_type=3",
        ),
        wgp3=Param(
            int,
            1,
            msg="Integer specifying the local geometry for the Witt & Gordon (2000) "
            "dust models, used only when dust_type=3",
        ),
        dust_emission_gamma_key=Param(
            str,
            "dust_gamma",
        ...
        fraction_agn_bol_lum_key=Param(
            str,
            "f_agn",
            msg="Fractional contributions of AGN wrt stellar bolometric"
            " luminosity, dataset keyword name",
        ),
        agn_torus_opt_depth_key=Param(
            str,
            "tau_agn",
            msg="Optical depths of the AGN dust torii" " dataset keyword name",
        ),
        tabulated_sfh_key=Param(
            str, "tabulated_sfh", msg="tabulated SFH dataset keyword name"
        ),
    ...
    def __init__(self, args, **kwargs):
        ...
        if self.config.zcontinuous not in [0, 1, 2, 3]:  # pragma: no cover
            raise ValueError(
                "zcontinous={} is not valid, allowed values are 0,1,2,3".format(
                    self.config.zcontinuous
                )
            )

        if self.config.imf_type not in [0, 1, 2, 3, 4, 5]:  # pragma: no cover
            raise ValueError(
                "imf_type={} is not valid, allowed values are 0,1,2,3,4,5".format(
                    self.config.imf_type
                )
            )

        if self.config.sfh_type not in [0, 1, 3, 4, 5]:  # pragma: no cover
            raise ValueError(
                "sfh_type={} is not valid, allowed values are 0,1,2,3,4,5".format(
                    self.config.sfh_type
                )
            )

        if self.config.dust_type not in [0, 1, 2, 3, 4, 5, 6]:  # pragma: no cover
            raise ValueError(
                "dust_type={} is not valid, allowed values are 0,1,2,3,4,5,6".format(
                    self.config.dust_type
                )
            )
    ...
    def _get_rest_frame_seds(
        ...
        for i in range(len(tage)):
            sp = fsps.StellarPopulation(
                compute_vega_mags=self.config.compute_vega_mags,
                ...
                cloudy_dust=self.config.cloudy_dust,
                agb_dust=self.config.agb_dust,
                tpagb_norm_type=self.config.tpagb_norm_type,
                dell=self.config.dell,
                delt=self.config.delt,
                redgb=self.config.redgb,
                agb=self.config.agb,
                fcstar=self.config.fcstar,
                sbss=self.config.sbss,
                fbhb=self.config.fbhb,
                pagb=self.config.pagb,
                zred=zred[i],
                zmet=zmet[i],
                logzsol=logzsol[i],
                pmetals=pmetals[i],
                imf_type=self.config.imf_type,
                imf_upper_limit=self.config.imf_upper_limit,
                imf_lower_limit=self.config.imf_lower_limit,
                imf1=self.config.imf1,
                imf2=self.config.imf2,
                imf3=self.config.imf3,
                vdmc=self.config.vdmc,
                mdave=self.config.mdave,
                evtype=self.config.evtype,
                use_wr_spectra=self.config.use_wr_spectra,
                logt_wmb_hot=self.config.logt_wmb_hot,
                masscut=self.config.masscut,
                sigma_smooth=sigma_smooth[i],
                min_wave_smooth=self.config.min_wavelength,
                max_wave_smooth=self.config.max_wavelength,
                gas_logu=gas_logu[i],
                gas_logz=gas_logz[i],
                igm_factor=self.config.igm_factor,
                sfh=self.config.sfh_type,
                tau=tau[i],
                const=const[i],
                sf_start=sf_start[i],
                sf_trunc=sf_trunc[i],
                ...
                tburst=tburst[i],
                sf_slope=sf_slope[i],
                dust_type=self.config.dust_type,
                dust_tesc=self.config.dust_tesc,
                dust1=dust1[i],
                dust2=dust2[i],
                dust_clumps=self.config.dust_clumps,
                frac_nodust=self.config.frac_nodust,
                frac_obrun=self.config.frac_obrun,
                dust_index=dust_index[i],
                dust1_index=dust1_index[i],
                mwr=mwr[i],
                uvb=uvb[i],
                wgp1=wgp1[i],
                wgp2=self.config.wgp2,
                wgp3=self.config.wgp3,
                duste_gamma=duste_gamma[i],
                duste_umin=duste_umin[i],
            ...
            if self.config.smooth_lsf:
                assert (
                    self.config.smooth_velocity is True
                ), "lsf smoothing only works if smooth_velocity is True"
                ...
                sp.set_lsf(
                    wave,
                    sigma,
                    wmin=self.config.min_wavelength,
                    wmax=self.config.max_wavelength,
                )  # pragma: no cover

            restframe_wavelength, restframe_sed_Lsun_Hz = sp.get_spectrum(
                tage=tage[i], peraa=False
            )

            selected_wave_range = np.where(
                (restframe_wavelength >= self.config.min_wavelength)
            ...
            if self.config.physical_units:
                ...
                restframe_sed_erg_s_Hz = (
                    restframe_sed_Lsun_Hz[selected_wave_range] * solar_luminosity_erg_s
                )
    ...
    def _process_chunk(self, start, end, data, first):
        ...
        wavelengths, restframe_seds = self._get_rest_frame_seds(
            zred=redshifts,
            zmet=zmet,
            logzsol=stellar_metallicities,
            pmetals=pmetals,
            sigma_smooth=velocity_dispersions,
            gas_logu=gas_ionizations,
            gas_logz=gas_metallicities,
            tau=tau,
            const=const,
            sf_start=sf_start,
            sf_trunc=sf_trunc,
            ...
            fagn=frac_bol_lum_agn,
            agn_tau=agn_torus_opt_depths,
            tabulated_sfhs=tabulated_sfhs,
            tabulated_lsfs=tabulated_lsfs,
        )
        output_chunk = {
            self.config.restframe_wave_key: wavelengths,
