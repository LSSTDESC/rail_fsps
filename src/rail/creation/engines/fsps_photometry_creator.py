...
class FSPSPhotometryCreator(Creator):
    ...
    config_options.update(
        ...
        restframe_wave_key=Param(
            str,
            "wavelength",
            msg="Rest-frame wavelengths keyword name of the"
            "hdf5 dataset containing rest-frame SEDs",
        ),
        ...
        w0=Param(
            float, -1, msg="Dark energy equation-of-state parameter at current time"
        ),
        wa=Param(
            float,
            0.0,
            msg="Slope dark energy equation-of-state evolution with scale factor",
        ),
    ...
    def __init__(self, args, **kwargs):
        ...
        if (self.config.Om0 < 0.0) | (self.config.Om0 > 1.0):
            raise ValueError(
                "The mass density at the current time {self.config.Om0} is outside of allowed"
                " range 0. < Om0 < 1."
            )
        if (self.config.Ode0 < 0.0) | (self.config.Ode0 > 1.0):
            raise ValueError(
                "The dark energy density at the current time {self.config.Ode0} is outside of allowed"
                " range 0. < Ode0 < 1."
            )
        if (self.config.h < 0.0) | (self.config.h > 1.0):
            raise ValueError(
                "The dimensionless Hubble constant {self.config.h} is outside of allowed"
                " range 0 < h < 1"
            )
    ...
    def _compute_apparent_magnitudes(
        self, rest_frame_wavelengths, rest_frame_seds, redshifts
    ):
        """
        ...
        for i in self.split_tasks_by_rank(range(len(redshifts))):
            ...
            lum_dist_cm = (
                self.cosmology.luminosity_distance(redshifts[i]).value * Mpc_in_cm
            )
            ...
            observedframe_wavelength_in_Hz = (
                speed_of_light_cm_s / observedframe_wavelength
            )

            magnitudes = []

            for j in range(len(self.filter_transmissions)):
                filter_wavelength_in_hz = (
                    speed_of_light_cm_s / self.filter_wavelengths[j]
                )
                interp_function = interpolate.interp1d(
                    filter_wavelength_in_hz,
                    self.filter_transmissions[j],
                    bounds_error=False,
                    fill_value=0,
                )
        ...
        if self.comm is not None:  # pragma: no cover
            ...
            apparent_magnitudes = {
                k: v for a in apparent_magnitudes for k, v in a.items()
            }

        apparent_magnitudes = np.array(
            [apparent_magnitudes[i] for i in range(len(redshifts))]
        )
    ...
    def run(self):
        ...
        apparent_magnitudes = self._compute_apparent_magnitudes(
            rest_frame_wave, rest_frame_seds, redshifts
        )
