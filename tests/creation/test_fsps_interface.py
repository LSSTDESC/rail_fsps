...
if "SPS_HOME" not in os.environ:
    ...
    subprocess.run(
        [
            "git",
            "clone",
            "https://github.com/cconroy20/fsps.git",
            "/opt/hostedtoolcache/Python/fsps",
        ],
        capture_output=True,
    )
...
RAIL_FSPS_DIR = os.path.abspath(
    os.path.join(os.path.dirname(rail.fsps.__file__), "..", "..")
)
default_rail_fsps_files_folder = os.path.join(
    RAIL_FSPS_DIR, "rail", "examples_data", "creation_data", "data", "fsps_default_data"
)
        ...
        (
            {
                "filter_folder": os.path.join(
                    default_rail_fsps_files_folder, "test_fsps_sed.fits"
                )
            },
            OSError,
        ),
...
def test_FSPSPhotometryCreator_bad_filter_folder(settings, error):
    ...
    with pytest.raises(error):
        fsps_photometry_creator.FSPSPhotometryCreator.make_stage(
            name="FSPSPhotometryCreator", **settings
        )
